...
function getModuleNameOfIdentifier(context, identifier) {
    ...
    const importDeclaration = getImportDeclarations(context).find(importDecl => ast_shape_1.isNamespaceSpecifier(importDecl, name) || ast_shape_1.isDefaultSpecifier(importDecl, name));
    if (importDeclaration) {
        return importDeclaration.source;
    }
    // check if importing using `const X = require('module_name')`
    const writeExpression = node_extractors_1.getUniqueWriteUsage(context, name);
    if (writeExpression) {
...
function getModuleNameOfImportedIdentifier(context, identifier) {
    ...
    const writeExpression = node_extractors_1.getUniqueWriteUsage(context, identifier.name);
    if (writeExpression &&
        writeExpression.type === 'MemberExpression' &&
        ast_shape_1.isIdentifier(writeExpression.property, identifier.name)) {
...
function getModuleNameFromRequire(node) {
    if (node.type === 'CallExpression' &&
        ast_shape_1.isIdentifier(node.callee, 'require') &&
...
function isCallToFQN(context, callExpression, moduleName, functionName) {
    ...
    return (module === null || module === void 0 ? void 0 : module.value) === moduleName && ast_shape_1.isIdentifier(callee.property, functionName);
}
exports.isCallToFQN = isCallToFQN;
//# sourceMappingURL=module-resolving.js.map